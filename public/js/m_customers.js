...
function getList(){
	$.ajax({
		url:mRoute('admin_ajax/customers/getList'),
		type:'get',
		dataType:'json',
		timeout:30000,
		data:{
...
function add(){
	var str = '<div class="js_dialog" id="iosDialog1" style="opacity: 1;">'+
		            ...
		            '<div class="weui-dialog">'+
		                '<div class="weui-dialog__hd"><strong class="weui-dialog__title">新建公司</strong></div>'+
		                '<div class="weui-dialog__bd">'+
		                	'<p>'+
		                		'<span>公司:</span><input name="name" type=text placeholder="公司名">'+
		                	'</p>'+
		                	'<p class="abb">'+
...
function del(obj){
	...
	var str = '<div class="js_dialog" id="iosDialog1" style="opacity: 1;">'+
		            ...
		            '<div class="weui-dialog">'+
		                ...
		                '<div class="weui-dialog__bd" style="margin-top:0px;height:auto;font-size: 16px;">确定删除此公司？</div>'+
		                '<div class="weui-dialog__ft">'+
...
function create(){
	...
	$.ajax({
		url:mRoute('admin_ajax/customers/createCpy'),
		type:'post',
		timeout:30000,
		data:{
			'name':name,
		...
		success:function(res){
			...
			if(res&&res.code==-1){
				...
				window.location.href = mRoute('admin/mainCustomer/'+abb);
			}else if(res&&res.code==-100){
				reload(res.msg);
			}
		}
...
function delSub(abb){
	...
	console.log(abb);
	$.ajax({
		url:mRoute('admin_ajax/customers/del'),
		type:'delete',
		dataType:'json',
		timeout:30000,
		data:{
...
function search(){
	...
	if(keyword==''){
		$.ajax({
			url:mRoute('admin_ajax/customers/index_list'),
			type:'get',
			timeout:30000,
			success:function(res){
				$('body').attr('data-scroll',0);
...
function searchKeywords(){
	...
	$.ajax({
		url:mRoute('admin_ajax/customers/search'),
		type:'get',
		dataType:'json',
		timeout:30000,
		data:{
...
function pageSort(){
	...
	$.ajax({
		url:mRoute('admin_ajax/customers/sort'),
		type:'get',
		dataType:'json',
		timeout:30000,
		data:{
