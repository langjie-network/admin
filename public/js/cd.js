...
$('input[type=file]').change(function(event) {
        if ($('input[type=file]').val().length) {
            ...
            if (extension == ".jpg" || extension == ".png" || extension == ".jpeg") {
                    ...
                    data.append('cpy', $('tbody[data-cpy]').attr('data-cpy'));

                    $.ajax({
                        url: router('customer/uploadImg'),
                        type: 'POST',
                        data: data,
						dataType:"json",
                        cache: false,
 ...
 var cdCover = document.getElementById("cd-cover");
	cdCover.onclick=function(){
		...
		if(num!=1){
			...
			var cpy=$('tbody[data-cpy]').attr('data-cpy');
			data={"pic":arr[0],"cpy":cpy};
			$.ajax({
				url: router('customer/cover'),
				type: 'POST',
				data: data,
				dataType:'json',
				cache: false,
