 ...
 * */
     ...
     * 上传图片并展示
     */
$('.cd-input input[type=file]').change(function(event) {
        if ($('.cd-input input[type=file]').val().length) {
            var fileName = $('.cd-input input[type=file]').val();
            ...
            if (extension == ".jpg" || extension == ".png" || extension == ".jpeg") {
                    ...
                    $.ajax({
                        url: router('knowledge/uploadImg'),
                        type: 'POST',
                        data: data,
						dataType:"json",
                        cache: false,
 ...
 var cdCover = document.getElementById("cd-cover");
	cdCover.onclick=function(){
		...
		if(num!=1){
			...
			var id = $('body').attr('data-id');
			data={"pic":arr[0],"id":id};
			$.ajax({
				url: router('knowledge/cover'),
				type: 'POST',
				data: data,
				dataType:'json',
				cache: false,
