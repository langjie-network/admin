 ...
 * */
    ...
    function cdIn(){
        var cdmodal=document.getElementsByClassName("cd-modal")[0];
			...
			data={"user_id":$('input[data-key=user_id]').val()};
			
			 $.ajax({
                        url: router('users/getImg'),
                        type: 'GET',
                        data: data,
						dataType:'json',
                        cache: false,
...
$('input[type=file]').change(function(event) {
        if ($('input[type=file]').val().length) {
            ...
            if (extension == ".jpg" || extension == ".png" || extension == ".jpeg") {
                    ...
                    data.append('user_id', $('input[data-key=user_id]').val());

                    $.ajax({
                        url: router('users/uploadImg'),
                        type: 'POST',
                        data: data,
						dataType:"json",
                        cache: false,
...
var cddelete=document.getElementById("cd-delete");
	cddelete.onclick=function(){
		var delImg=[];
		var cpy=$('input[data-key=user_id]').val();
