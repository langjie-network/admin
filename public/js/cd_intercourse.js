 ...
 * */
    ...
    function cdIn(){
        var cdmodal=document.getElementsByClassName("cd-modal")[0];
			...
			data = {
				"no": $('body').attr('data-id')
			};
			 $.ajax({
                        url: router('intercourse/getImg'),
                        type: 'GET',
                        data: data,
						dataType:'json',
                        cache: false,
...
$('input[type=file]').change(function(event) {
        if ($('input[type=file]').val().length) {
            ...
            if (extension == ".jpg" || extension == ".png" || extension == ".jpeg") {
                    ...
                    $.ajax({
                        url: router('intercourse/uploadImg'),
                        type: 'POST',
                        data: data,
						dataType:"json",
                        cache: false,
                        ...
                        success: function(data) {
									toast(data.msg);
									var album=data.data[0].action_img;
									var arr=album.split(",")
