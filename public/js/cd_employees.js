 ...
 * */
    ...
    function cdIn(){
        var cdmodal=document.getElementsByClassName("cd-modal")[0];
			...
			data={"user_id":$('tbody[data-id]').attr('data-id')};
			 
			 $.ajax({
                        url: router('employee/getImg'),
                        type: 'GET',
                        data: data,
						dataType:'json',
                        cache: false,
...
$('input[type=file]').change(function(event) {
        if ($('input[type=file]').val().length) {
            ...
            if (extension == ".jpg" || extension == ".png" || extension == ".jpeg") {
                    ...
                    data.append('user_id', $('tbody[data-id]').attr('data-id'));
                    $.ajax({
                        url: router('employee/uploadImg'),
                        type: 'POST',
                        data: data,
						dataType:"json",
                        cache: false,
...
var cddelete=document.getElementById("cd-delete");
	cddelete.onclick=function(){
		var delImg=[];
		var user_id=$('tbody[data-id]').attr('data-id');
		...
		if(delImg.length!=0){
			...
			data={'pic':a,"user_id":user_id};
			console.log(data)
				$.ajax({
					url: router('employee/delImg'),
					type: 'POST',
					data: data,
					dataType:'json',
					cache: false,
 ...
 var cdCover = document.getElementById("cd-cover");
	cdCover.onclick=function(){
		...
		if(num!=1){
			...
			var user_id=$('tbody[data-id]').attr('data-id');
			data={"pic":arr[0],"user_id":user_id};
			console.log(data);
			$.ajax({
				url: router('employee/cover'),
				type: 'POST',
				data: data,
				dataType:'json',
				cache: false,
